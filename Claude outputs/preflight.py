...
def check_email():
    ...
    # THIS CHECK WAS WRONG AND REPORTED A FALSE FAILURE.
    #
    # It looked for SPF on the ROOT domain only. Resend does not put it
    # there. SPF authenticates the ENVELOPE sender (the Return-Path / SMTP
    # MAIL FROM), not the From: header, and Resend's Return-Path is on the
    # `send.` subdomain — which is exactly why their setup has you create
    # BOTH `send` TXT (v=spf1 include:amazonses.com ~all) and `send` MX
    # (feedback-smtp...amazonses.com) rather than touching the root.
    #
    # So a completely correct Resend configuration has no SPF record at the
    # root at all, and this check called that a FAIL and told the user their
    # sign-in mail would not authenticate. It authenticates fine: SPF passes
    # on send.<domain> and aligns with the From: domain under DMARC's
    # relaxed alignment (same organisational domain), and DKIM aligns
    # outright.
    #
    # Check both, and pass on either.
    spf_root, err = dns_txt(DOMAIN)
    if err:
        say(WARN, f"could not run nslookup ({err}); check DNS by hand")
        return
    spf_send, _ = dns_txt("send." + DOMAIN)
    has_root = any("v=spf1" in r for r in (spf_root or []))
    has_send = any("v=spf1" in r for r in (spf_send or []))
    if has_send:
        say(OK, "SPF record present on send." + DOMAIN
                + " (Resend's envelope domain — this is the one that matters)")
    elif has_root:
        say(OK, "SPF record present on the root domain")
    else:
        say(BAD, "NO SPF record at " + DOMAIN + " or send." + DOMAIN
                 + ". Resend mail will not authenticate as your domain.")
...
def check_site(expect_version):
    ...
    for path, what in (("/", "index"), ("/sw.js", "service worker")):
        ...
        try:
            # A BROWSER User-Agent, deliberately, and this is not cosmetic.
            #
            # urllib sends "Python-urllib/3.x" by default. Cloudflare's bot
            # protection answers that with 403 Forbidden, so this check
            # reported BOTH the site and the service worker as unreachable
            # on a site that was up, serving correctly, and being used by
            # real players at that moment. Two FAILs, no fault.
            #
            # The point of this check is "is the deploy live and is it the
            # version I just pushed", which is a question about the origin,
            # not about how Cloudflare grades scripted clients. Ask it the
            # way a player's phone would.
